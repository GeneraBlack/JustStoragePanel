...
public final class CraftingPanelJeiTransferHandler implements IRecipeTransferHandler<CraftingPanelMenu, RecipeHolder<CraftingRecipe>> {
    ...
    public IRecipeTransferError transferRecipe(CraftingPanelMenu container, RecipeHolder<CraftingRecipe> recipe, IRecipeSlotsView recipeSlots, Player player, boolean maxTransfer, boolean doTransfer) {
        Map<Integer, SlotDisplay> slotMap = this.transferHelper.getGuiSlotIndexToIngredientMap(recipe);
        ...
        for (int slot = 0; slot < 9; slot++) {
            SlotDisplay slotDisplay = slotMap.get(slot);
            if (slotDisplay == null) {
                ingredients.add(ItemStack.EMPTY);
            } else {
                List<ItemStack> candidates = slotDisplay.resolveForStacks(context);
                ingredients.add(findBestMatchingItem(container, slot, candidates));
            }
        }
    ...
    private static ItemStack findBestMatchingItem(CraftingPanelMenu container, int slotIndex, List<ItemStack> candidates) {
        ...
        if (craftSlot < container.slots.size()) {
            ...
            if (matchesAny(inGrid, candidates)) {
        ...
        for (int i = container.getPlayerInventoryStart(); i < container.getPlayerInventoryEnd(); i++) {
            ...
            if (matchesAny(inInv, candidates)) {
        ...
        for (int i = 0; i < CraftingPanelMenu.DISPLAY_SLOT_COUNT; i++) {
            ...
            if (matchesAny(inDisplay, candidates)) {
        ...
        // 4. Fallback to candidate's first valid item
        ItemStack copy = candidates.get(0).copy();
        copy.setCount(1);
        return copy;
    }
    ...
    private static boolean matchesAny(ItemStack stack, List<ItemStack> candidates) {
        ...
        for (ItemStack candidate : candidates) {
            if (ItemStack.isSameItemSameComponents(stack, candidate) || ItemStack.isSameItem(stack, candidate)) {
                return true;
            }
        }
        return false;
