...
| Current release line | 26.1.2 | 26.1.2.41-beta | release/26.1.2 | 1.4.x | Primary maintained line; Java 25 and the new 26.1 GUI/input APIs. |

## Current Recommendation

1. Cut new public releases from release/26.1.2.
2. Keep 26.1.2 as the primary maintained line for current public releases.
3. Keep 1.21.11 only for targeted hotfixes when a downstream pack or server cannot move yet.
4. Keep 1.21.1 as the long-tail fallback branch; only revisit 1.21.5 or 1.21.8 if a concrete compatibility demand appears.
5. Reassess 26.2 snapshots separately; do not mix snapshot planning into the stable 26.1.2 release branch.

## Tooling Notes

- The 26.1.2 line uses ModDevGradle 2.0.141.
- Java 25 is required locally and in CI for this line.
- Parchment is not used for 26.1.2.
...
- Dedicated server startup is part of the validation baseline for 26.1.2 because the storage network is intentionally multiplayer-safe and server-driven.
...
| minecraft_version | 26.1.2 | gradle.properties |
| minecraft_version_range | [26.1.2] | gradle.properties |
| neo_version | 26.1.2.41-beta | gradle.properties |
| jei_version | 29.5.0.28 | gradle.properties |
| loader_version_range | [1,) | gradle.properties |
| mod_version for first 26.1.2 release | 1.4.0 | gradle.properties |

## 26.1.2 Validation Baseline

- `compileJava processResources --no-configuration-cache` completes successfully on JDK 25.
- `runClient` starts successfully with JEI present and the Just Storage Panel JEI plugin initialized.
- `runServer` reaches the dedicated server ready state without a server-side classloading or payload registration failure.
- The panel GUIs use `GuiGraphicsExtractor` with the new `extractBackground`, `extractLabels`, and `extractSlot` hooks.
...
2. Confirm build tooling is correct for the target line; for 26.1.2 this means ModDevGradle, Java 25, and no Parchment block.
...
5. Run `runServer` whenever server-sensitive code changed or when preparing a public 26.1.2 release.
